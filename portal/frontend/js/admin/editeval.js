			...
			.validate(
					{
						...
						submitHandler : function(form) {
							// var data = getInfoAjax('saveEvaluacion',
							// $(form).serialize()),
							var id = $("#A_t_Neval").attr('eval');
							;
							var data = getInfoAjax('updateEvaluacion/' + id, $(
									form).serialize(), "admin");
							// titulo = 'Nueva evaluación',
							// mns = data.error || data.mensaje.msg;
							// console.log(data);
							// window.location.href = data.mensaje.url;
							// $( "#back" ).trigger( "click" );
							// window.location.href = '../evaluaciones';
							// return false;
							// mostrarAlert(titulo, mns);

							// if (data.error) {
							// botonOkModal(false)
							// } else {
							// botonOkModal(true, data.mensaje.url)
							// }
							if (data.error) {
